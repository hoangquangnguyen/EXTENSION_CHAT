...
function sendViaWebSocket(payload) {
  const enrichedPayload = {
    ...payload,
    ai_reply_id: payload.ai_reply_id || connectionSettings.ai_reply_id || "default"
  };
  if (socket && socket.readyState === WebSocket.OPEN) {
    try {
      socket.send(JSON.stringify(enrichedPayload));
      console.log("Offscreen: Sent comment via WebSocket:", enrichedPayload);
    } catch (error) {
      console.error("Offscreen: Error sending via WebSocket:", error);
      queueComment(enrichedPayload);
    }
  } else {
...
function sendViaHttpPost(payload) {
  ...
  console.log(`Offscreen: Sending HTTP POST to ${url} for AI [${enrichedPayload.ai_reply_id}]`);
